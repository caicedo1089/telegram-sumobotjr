    ...
    {   
        ...
        for(let i=0; i<allCommands.length; ++i)
        {
            ...
            try 
            {
                ...
                objAllCmds[command] = new instanceCommand(command, bot, sumoBot, this)
            ...
            function(msg)
            {
                ...
                if(msg.entities)
                {
                    ...
                    if(entitieCommand.type == 'bot_command')
                    {
                        ...
                        {
                            if(!_this.maintenance || command=='config')
                            {
                                objAllCmds[command].excecute(chatId, msg)    
                            }
                            else
                            {
                                bot.sendMessage(chatId, _this.lang('Estamos en mantenimiento.\nVolveremos pronto...'))
                            }
                        }
                        else
                        {
                            bot.sendMessage(chatId, _this.lang('Comando inexistente'))
                        }
                    }
                    else
                    {
                        bot.sendMessage(chatId, _this.lang('Comando no encontrado'))
                    }
                }
                else
                {
                    ...
                    let command = _this.session[chatId].command || 'start'
